...
async def initiate_post_creation(message: Message, state: FSMContext):
...
async def receive_post_topic(message: Message, state: FSMContext, bot: Bot):
    if message.text == "Отмена":
        await message.answer("Действие отменено!", reply_markup=ReplyKeyboardRemove())
        await state.clear()
        await handle_start_command(message)
...
async def handle_like_dislike(callback_query: CallbackQuery, bot: Bot):
